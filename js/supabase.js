...
(function(){
  try{
    ...
    }catch(e){window.ECOMAX_STORAGE_ERROR=(e&&e.message)||String(e);}
    let client;
    try{
      client=window.supabase.createClient(SUPABASE_URL,SUPABASE_ANON_KEY,{auth:{storageKey:ECOMAX_AUTH_STORAGE,storage:window.localStorage,persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:"pkce"}});
    }catch(e){
      window.ECOMAX_SUPABASE_ERROR=(e&&e.message)||String(e);
      console.error("ECOMAX Supabase createClient error:",e);
      return;
    }
    if(!client||!client.auth){window.ECOMAX_SUPABASE_ERROR="Supabase client შეიქმნა არასწორად";return;}
    window.ECOMAX_SUPABASE_CLIENT=client;
    window.ECOMAX_AUTH_CLIENT=client;
    const originalCreateClient=window.supabase.createClient;
    window.supabase.createClient=function(){return window.ECOMAX_SUPABASE_CLIENT||originalCreateClient.apply(window.supabase,arguments);};
    async function stableSession(){
      for(let i=0;i<40;i++){
        ...
        }catch(e){window.ECOMAX_AUTH_SESSION_ERROR=(e&&e.message)||String(e);}
    ...
    client.auth.onAuthStateChange(function(event,session){
      ...
      window.ECOMAX_CURRENT_USER=session&&session.user||null;
      try{
        if(session){const raw=localStorage.getItem(ECOMAX_AUTH_STORAGE);if(raw)localStorage.setItem(ECOMAX_LEGACY_STORAGE,raw);}
        else if(event==="SIGNED_OUT"){localStorage.removeItem(ECOMAX_AUTH_STORAGE);localStorage.removeItem(ECOMAX_LEGACY_STORAGE);}
      }catch(e){}
    });
  }catch(e){
    window.ECOMAX_SUPABASE_ERROR=(e&&e.message)||String(e);
    console.error("ECOMAX Supabase bootstrap error:",e);
  }
})();
(function(){
