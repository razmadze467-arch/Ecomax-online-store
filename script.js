...
function forceMaxHomeDesign(){
  ...
  const s=document.createElement("script");s.id="ecomaxMaxDesignJs";s.src="js/home-max-design.js?v=20260916-2";s.defer=false;document.head.appendChild(s);
}
...
function init(){
  ...
  const menuButton=document.getElementById("menuButton")||document.getElementById("mobileMenu"),mobileNav=document.getElementById("mobileNav");if(menuButton&&mobileNav){menuButton.addEventListener("click",event=>{event.preventDefault();event.stopPropagation();mobileNav.classList.toggle("active");mobileNav.style.display=mobileNav.classList.contains("active")?"block":"none";});mobileNav.querySelectorAll("a").forEach(a=>a.addEventListener("click",closeMobileMenu));}
